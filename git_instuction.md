...
Чтобы переключиться на другую ветку неоходимо выполнить уже знакjмую нам команду
*git checkout*. При этом стоит стоит понимать, что переключиться без фиксации изменений
в репозитории Git нам не позволит. 

### Слияние веток

Чтобы внести изменения из одной ветки в другую используют команду

    git merge <branch_name>

**Важно:** *выполняют эту команду находясь на состоянии (commit) и ветке (branch),
начиная с которого эти изменения необходимо учесть.* 


#### Конфликт слияния

Иногда при написании одной строки мы сталкиваемся с ситуацией, при которой внесенные
изменения затрагивают одни и те же строки. Некоторые такие ситуации Git устраняет
самостоятельно, руководствуясь встроенными стратегиями, другие приходится разрешать
вручную.


А теперь попробуем вызвать конфликт слияния, требующий ручной правки
